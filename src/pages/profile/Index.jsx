...
const Profile = () => {
  ...
  const handleNavigate = () =>{
    if(userInfo.profileSetup){
      ...
      toast.error("Complete your setup")
    }
  }
  ...
  return (
    <div className="bg-[#1b1c24] h-[100vh] flex items-center justify-center flex-col gap-10">
      <div className="flex flex-col gap-10 w-[80vw] md:w-max ">
        <div className="">
          <IoArrowBack className='text-4xl lg:text-6xl text-white/90 cursor-pointer' onClick={handleNavigate}></IoArrowBack>
        </div>
        <div className="grid grid-cols-2">
          ...
          <div className="flex min-w-32 md:min-w-64 flex-col gap-5 text-white items-center
          justify-center">
            ...
            <div className="w-full">
              <Input placeholder="first Name" type="text" onChange={(e) => {setFirstName(e.target.value)}} value={firstName}
              className="rounded-lg p-6 bg-[#2c2e3b] border-none"></Input>
            </div>
            <div className="w-full">
            ...
            <div className="w-full flex gap-6">
              {
                colors.map((color, index) => <div className={`${color} h-8 w-8 rounded-full cursor-pointer transition-all
                duration-300 ${selectColor === index ? "outline outline-white/50 outline-1" : ""}`} key={index}
                onClick={() => {setSelectColor(index)}}></div>
                )}
            </div>
          </div>
        </div>
